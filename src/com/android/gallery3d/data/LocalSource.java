...
class LocalSource extends MediaSource {
    ...
    public static final int LOCAL_ALL_ALBUMSET = 6;
    public static final int LOCAL_SELECTION_ALBUMSET = 8;
    public static final int LOCAL_SETTING_ALBUMSET = 9;
    public static final int LOCAL_CUSTOM_ALBUMSET = 10;
    public static final Path EMPTY_ALBUM_PATH = Path.fromString("/local/empty");
    ...
    // public  6 & 8 & 9 & 10
    private static final int LOCAL_ALL_VIDEO_ALBUMSET = 11;
    private static final int LOCAL_ALL_PANORAMAS_ALBUMSET = 12;
    ...
    public LocalSource(GalleryApp context) {
        ...
        //custom source
        mMatcher.add("/local/selection", LOCAL_SELECTION_ALBUMSET);
        mMatcher.add("/local/setting", LOCAL_SETTING_ALBUMSET);
        mMatcher.add("/local/video/all", LOCAL_ALL_VIDEO_ALBUMSET);
        mMatcher.add("/local/panoramas/all", LOCAL_ALL_PANORAMAS_ALBUMSET);
        mMatcher.add("/local/custom", LOCAL_CUSTOM_ALBUMSET);
    ...
    private static int getMediaType(String type, int defaultType) {
        ...
        try {
            ...
            Logger.w(TAG, "invalid type: " + type, e);
        }
        return defaultType;
    }
    ...
    public MediaObject createMediaObject(Path path) {
        ...
        Logger.d(TAG,"path: " + path.toString());
        switch (mMatcher.match(path)) {
            case LOCAL_ALL_ALBUMSET:
            case LOCAL_IMAGE_ALBUMSET:
            case LOCAL_VIDEO_ALBUMSET:
                return new LocalAlbumSet(path, mApplication, LOCAL_ALL_ALBUMSET);
            case LOCAL_CUSTOM_ALBUMSET:
                return new LocalAlbumSet(path, mApplication, LOCAL_CUSTOM_ALBUMSET);
            case LOCAL_ALL_VIDEO_ALBUMSET:
                return new LocalVideoAlbumSet(path, mApplication);
            case LOCAL_ALL_PANORAMAS_ALBUMSET:
                return new LocalPanoramasAlbumSet(path, mApplication);
            case LOCAL_SETTING_ALBUMSET:
                return new LocalAlbumSet(path, mApplication, LOCAL_SETTING_ALBUMSET);
            case LOCAL_SELECTION_ALBUMSET:
                return new LocalAlbumSet(path, mApplication, LOCAL_SELECTION_ALBUMSET);
    ...
    private Path getAlbumPath(Uri uri, int defaultType) {
        int mediaType = getMediaType(uri.getQueryParameter(GalleryActivity.KEY_MEDIA_TYPES), defaultType);
        String bucketId = uri.getQueryParameter(KEY_BUCKET_ID);
        int id = 0;
        try {
            id = Integer.parseInt(bucketId);
        } catch (NumberFormatException e) {
            Logger.w(TAG, "invalid bucket id: " + bucketId, e);
            return null;
        }
        switch (mediaType) {
    ...
    public Path findPathByUri(Uri uri, String type) {
        try {
            ...
            Logger.w(TAG, "uri: " + uri.toString(), e);
        }
        return null;
    }
