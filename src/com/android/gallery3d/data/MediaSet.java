...
public abstract class MediaSet extends MediaObject {
    ...
    public static final int MEDIAITEM_BATCH_FETCH_TIME_COUNT = 1000;
     ...
     */
    public static boolean isShowTimeTitle() {
        return mShowTitle;
    }
     ...
     *                  false: we don't need TimeTitle
     */
    public static void setShowTimeTitle(boolean showTitle) {
    ...
    /**
     * get media item cover[1-3] for 3D
     *
     * @return
     */
    public MediaItem[] getCoverMediaItem2() {
        try {
            ArrayList<MediaItem> items = getMediaItem(0, GalleryUtils.SUPPORT_3D ? 3 : 1);
            if (items.size() > 0) return items.toArray(new MediaItem[]{});
            for (int i = 0, n = getSubMediaSetCount(); i < n; i++) {
                MediaItem[] cover = getSubMediaSet(i).getCoverMediaItem2();
                if (cover != null) return cover;
            }
        } catch (Throwable ex) {
    ...
    /**
     * get current album status
     *
     * @return
     */
    public int getStatus() {
        return MediaSet.ALBUM_FLAG_HIDDEN;
    }

    /**
     * get media set or media item absolute file path
     *
     * @return
     */
    ...
    protected int enumerateMediaItems(ItemConsumer consumer, int startIndex) {
        ...
        int start = 0;
        while (start < total) {
            int count = Math.max(MEDIAITEM_BATCH_FETCH_TIME_COUNT, total - start);
    ...
    private class MultiSetSyncFuture implements Future<Integer>, SyncListener {
        ...
        private static final String TAG = "MediaSet.MultiSetSync";
        ...
        MultiSetSyncFuture(MediaSet[] sets, SyncListener listener) {
            ...
            synchronized (this) {
                for (int i = 0, n = sets.length; i < n; ++i) {
                    ...
                    Logger.d(TAG, "  request sync: " + Utils.maskDebugInfo(sets[i].getName()));
                }
            }
        }
        ...
        public synchronized void waitDone() {
            try {
                ...
                Logger.d(TAG, "waitDone() interrupted");
            }
        }
        ...
        public void onSyncDone(MediaSet mediaSet, int resultCode) {
            ...
            synchronized (this) {
                ...
                Logger.d(TAG, "onSyncDone: " + Utils.maskDebugInfo(mediaSet.getName()) + " #pending=" + mPendingCount);
            }
