...
public class DataManager implements StitchingChangeListener {
    ...
    public static final int INCLUDE_SELECTION_ONLY = 11;
    public static final int INCLUDE_SETTING_ONLY = 21;
    public static final int INCLUDE_LOCAL_CAMERA_ALL = 31;
    public static final int INCLUDE_LOCAL_CAMERA_IMAGE = 41;
    public static final int INCLUDE_LOCAL_CAMERA_VIDEO = 51;

    public static final int INCLUDE_ALBUM_CUSTOM_ONLY = 22;
    ...
    private static final String TOP_SELECTION_PATH = "/combo/{/local/selection,/picasa/all}";

    private static final String TOP_SETTING_PATH = "/combo/{/local/setting,/picasa/all}";

    private static final String TOP_CUSTOM_PATH = "/combo/{/local/custom,/picasa/all}";
    ...
    private static final String TOP_LOCAL_CAMERA_PATH = "/local/all/" + MediaSetUtils.CAMERA_BUCKET_ID;

    private static final String TOP_LOCAL_IMAGE_CAMERA_PATH = "/local/image/" + MediaSetUtils.CAMERA_BUCKET_ID;

    private static final String TOP_LOCAL_VIDEO_CAMERA_PATH = "/local/video/" + MediaSetUtils.CAMERA_BUCKET_ID;
    ...
    public String getTopSetPath(int typeBits) {
        switch (typeBits) {
            ...
            case INCLUDE_ALL:
                return TOP_SET_PATH;
            case INCLUDE_ALBUM_CUSTOM_ONLY:
                return TOP_CUSTOM_PATH;
            ...
            case INCLUDE_SELECTION_ONLY:
                return TOP_SELECTION_PATH;
            case INCLUDE_SETTING_ONLY:
                return TOP_SETTING_PATH;
            case INCLUDE_LOCAL_CAMERA_ALL:
                return TOP_LOCAL_CAMERA_PATH;
            case INCLUDE_LOCAL_CAMERA_IMAGE:
                return TOP_LOCAL_IMAGE_CAMERA_PATH;
            case INCLUDE_LOCAL_CAMERA_VIDEO:
                return TOP_LOCAL_VIDEO_CAMERA_PATH;
            default:
    ...
    public MediaObject getMediaObject(Path path) {
        Logger.d(TAG, "getMediaObject - path: " + path.toString());
        synchronized (LOCK) {
            ...
            if (source == null) {
                Logger.w(TAG, "cannot find media source for path: " + path);
                return null;
            }
            ...
            try {
                ...
                if (object == null) {
                    Logger.w(TAG, "cannot create media object: " + path);
                }
                return object;
            } catch (Throwable t) {
                Logger.w(TAG, "exception in creating media object: " + path, t);
                return null;
            }
        }
    }
    ...
    public void update(Path path, Bundle bundle) {
        MediaObject mediaObject = getMediaObject(path);
        if (mediaObject != null) {
            mediaObject.update(bundle);
        }
    }
    ...
    public void registerChangeNotifier(Uri uri, ChangeNotifier notifier) {
        ...
        synchronized (mNotifierMap) {
            ...
            if (broker == null) {
                ...
                //Logger.d(TAG,"registerChangeNotifier - uri: " + uri.toString() + " notifier: " + notifier.toString());
    ...
    private static class NotifyBroker extends ContentObserver {
        private Map<ChangeNotifier, Object> mNotifiers = Collections.synchronizedMap(new WeakHashMap<ChangeNotifier, Object>());
