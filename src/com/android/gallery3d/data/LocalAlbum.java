...
public class LocalAlbum extends MediaSet {
    ...
    public LocalAlbum(Path path, GalleryApp application, int bucketId, boolean isImage, String name, int status) {
    ...
    public LocalAlbum(Path path, GalleryApp application, int bucketId, boolean isImage) {
        this(path, application, bucketId, isImage, 0);
    }

    public LocalAlbum(Path path, GalleryApp application, int bucketId, boolean isImage, int status) {
        this(path, application, bucketId, isImage, BucketHelper.getBucketName(application.getContentResolver(), bucketId), status);
    }
    ...
    public static MediaItem[] getMediaItemById(GalleryApp application, boolean isImage, ArrayList<Integer> ids) {
        ...
        if (cursor == null) {
            Logger.w(TAG, "query fail" + baseUri);
            return result;
        }
        try {
            int n = ids.size();
    ...
    public static String getLocalizedName(Resources res, int bucketId, String name) {
        ...
        /*else if (bucketId == MediaSetUtils.CAMERA_VIDEO_BUCKET_ID) {
            return res.getString(R.string.folder_video);
        } else if (bucketId == MediaSetUtils.PANORAMA_BUCKET_ID) {
            return res.getString(R.string.folder_panorama);
    ...
    public static String getRelativePath(int bucketId) {
        ...
        if (bucketId == MediaSetUtils.CAMERA_BUCKET_ID) {
            ...
            if (path == null) {
                Logger.w(TAG, "Relative path for bucket id: " + bucketId + " is not found.");
                relativePath = null;
            } else {
    ...
    public int getMediaItemCount() {
        if (mCachedCount == INVALID_COUNT) {
            ...
            try {
                ...
                //Logger.d(TAG, "getMediaItemCount count: " + mCachedCount);
            } finally {
                cursor.close();
            }
        }
    ...
    public int getSupportedOperations() {
        if (MediaSetUtils.isSystemAlbum(mBucketId)) {
            return SUPPORT_DELETE | SUPPORT_INFO;
        } else {
            return SUPPORT_DELETE | SUPPORT_RENAME | SUPPORT_INFO;
        }
    }
    ...
    public void update(Bundle bundle) {
        GalleryUtils.assertNotInRenderThread();
        if (bundle != null) {
            int status = bundle.getInt(GalleryUtils.EXTRA_ALBUM_STATUS, MediaSet.ALBUM_FLAG_HIDDEN);
            ContentValues contentValues = new ContentValues();
            contentValues.put(GalleryUtils.MEDIA_IMAGE_COLUMNS_IS_HIDDEN, status);
            int is = mResolver.update(mBaseUri, contentValues, mWhereClause, new String[]{String.valueOf(mBucketId)});

            Logger.d(TAG, "update : " + is + " : " + mName);
        }
    }
