...
public class LocalMergeAlbum extends MediaSet implements ContentListener {
    ...
    public Uri getContentUri() {
        ...
        if (ApiHelper.HAS_MEDIA_PROVIDER_FILES_TABLE) {
            return MediaStore.Files.getContentUri("external").buildUpon()
                    .appendQueryParameter(LocalSource.KEY_BUCKET_ID, bucketId).build();
        } else {
    ...
    public int getStatus() {
        return mSources.length == 0 ? MediaSet.ALBUM_FLAG_HIDDEN : mSources[0].getStatus();
    }

    @Override
    public String getAbsoluteFilePath() {
        String path = null;
        if (mSources.length == 0) return path;
        try {
            MediaItem coverMediaItem = mSources[0].getCoverMediaItem();
            if (coverMediaItem == null) coverMediaItem = mSources[1].getCoverMediaItem();
            String parentPath = coverMediaItem.getFilePath();
            path = parentPath.substring(0, parentPath.lastIndexOf(File.separator));
        } catch (Exception ex) {
            Logger.e(TAG, "getAbsoluteFilePath err: " + ex.getMessage());
        } finally {
            return path;
        }
    }
