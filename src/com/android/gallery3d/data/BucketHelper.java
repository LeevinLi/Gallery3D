...
public class BucketHelper {
  ...
  private static final String[] PROJECTION_BUCKET = {
      ...
      GalleryUtils.SUPPORT_SETTING_HIDDEN ? GalleryUtils.MEDIA_IMAGE_COLUMNS_IS_HIDDEN
  ...
  private static final int INDEX_IS_HIDDEN = 3;
  ...
  public static BucketEntry[] loadBucketEntries(JobContext jc, ContentResolver resolver, int type) {
  ...
  private static void updateBucketEntriesFromTable(JobContext jc, ContentResolver resolver,
      Uri tableUri, HashMap<Integer, BucketEntry> buckets) {
    Cursor cursor =
        resolver.query(tableUri, PROJECTION_BUCKET_IN_ONE_TABLE, BUCKET_GROUP_BY_IN_ONE_TABLE, null,
            null);
    if (cursor == null) {
      Log.w(TAG, "cannot open media database: " + tableUri);
      return;
    }
    Logger.d(TAG, "update data form file table - count: " + cursor.getCount());
    try {
      while (cursor.moveToNext()) {
        int bucketId = cursor.getInt(INDEX_BUCKET_ID);
        int dateTaken = cursor.getInt(INDEX_DATE_TAKEN);
        int status = GalleryUtils.SUPPORT_SETTING_HIDDEN ? cursor.getInt(INDEX_IS_HIDDEN)
            : MediaSet.ALBUM_FLAG_SHOW;
        BucketEntry entry = buckets.get(bucketId);
        if (entry == null) {
          entry = new BucketEntry(bucketId, cursor.getString(INDEX_BUCKET_NAME), status);
  ...
  private static BucketEntry[] loadBucketEntriesFromImagesAndVideoTable(JobContext jc,
  ...
  private static BucketEntry[] loadBucketEntriesFromFilesTable(JobContext jc,
    ...
    Logger.d(TAG, "load data form file table - count: " + cursor.getCount());
    ...
    try {
      while (cursor.moveToNext()) {
        if ((typeBits & (1 << cursor.getInt(INDEX_MEDIA_TYPE))) != 0) {
          ...
          int albumStatus = GalleryUtils.SUPPORT_SETTING_HIDDEN ? cursor.getInt(INDEX_IS_HIDDEN)
              : MediaSet.ALBUM_FLAG_SHOW;
          BucketEntry entry = new BucketEntry(bucketId, bucketName, albumStatus);
    ...
    Logger.d(TAG, "loadBucketEntriesFromFilesTable - time: " + (t2 - t1));
  ...
  private static String getBucketNameInTable(ContentResolver resolver, Uri tableUri, int bucketId) {
    ...
    Cursor cursor =
        resolver.query(uri, PROJECTION_BUCKET_IN_ONE_TABLE, "bucket_id = ?", selectionArgs, null);
    try {
  ...
  private static String isExitsFolderInTable(ContentResolver resolver, Uri tableUri, String path) {
    String selectionArgs[] = new String[] { path + "/%" };
    Cursor cursor = null;
    try {
      Uri uri = tableUri.buildUpon().appendQueryParameter("limit", "1").build();
      cursor =
          resolver.query(uri, PROJECTION_BUCKET_IN_ONE_TABLE, "_data like ?", selectionArgs, null);
      if (cursor != null && cursor.moveToNext()) {
        return cursor.getString(INDEX_BUCKET_NAME);
      }
    } finally {
  ...
  /**
   * folder is empty by path
   */
  public static boolean isEmptyAlbum(ContentResolver resolver, String path) {
    if (ApiHelper.HAS_MEDIA_PROVIDER_FILES_TABLE) {
      String result = isExitsFolderInTable(resolver, getFilesContentUri(), path);
      return TextUtils.isEmpty(result);
    } else {
      String result = isExitsFolderInTable(resolver, Images.Media.EXTERNAL_CONTENT_URI, path);
      if (result != null) return TextUtils.isEmpty(result) ? false : true;
      result = isExitsFolderInTable(resolver, Video.Media.EXTERNAL_CONTENT_URI, path);
      return TextUtils.isEmpty(result);
    }
  }

  public static String getPathByUri(ContentResolver resolver, Uri uri) {
    if (uri == null) return null;
    Uri queryUri = uri.buildUpon().appendQueryParameter("limit", "1").build();
    Cursor cursor = resolver.query(queryUri, new String[] { ImageColumns.DATA }, null, null, null);
    try {
      if (cursor != null && cursor.moveToNext()) {
  ...
  public static String getDisplayNameByUri(ContentResolver resolver, Uri uri) {
    if (uri == null) return null;
    Uri queryUri = uri.buildUpon().appendQueryParameter("limit", "1").build();
    Cursor cursor =
        resolver.query(queryUri, new String[] { ImageColumns.DISPLAY_NAME }, null, null, null);
    try {
      if (cursor != null && cursor.moveToNext()) {
        return cursor.getString(cursor.getColumnIndex(ImageColumns.DISPLAY_NAME));
      }
    } finally {
      Utils.closeSilently(cursor);
    }
  ...
  public static class BucketEntry {
    ...
    public BucketEntry(int id, String name, int hidden) {
      bucketId = id;
