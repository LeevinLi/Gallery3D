...
public class TimeClustering extends Clustering {
    ...
    public void run(MediaSet baseSet) {
        ...
        int count = baseSet.getTotalMediaItemCount();
        final int total = count >= 0 ? count : 0;
        Logger.d(TAG, "media item total count: " + total);
    ...
    // compute by day, items of same year and same month and day are into same cluster.
    private void computeByDay(SmallItem currentItem) {
        if (currentItem != null) {
            int numCurrClusterItems = mCurrCluster.size();
            boolean itemAddedToCurrentCluster = false;
            ...
            if (numCurrClusterItems == 0) {
                mCurrCluster.addItem(currentItem);
                updateClusterTime(currentItem);
            } else {
                if (currentItem.dateInMs >= preTime && currentItem.dateInMs < endTime) {
                    mCurrCluster.addItem(currentItem);
                    itemAddedToCurrentCluster = true;
                } else {
                    mClusters.add(mCurrCluster);
    ...
    private void updateClusterTime(SmallItem currentItem) {
        try {
            preTime = DateUtils.getTodayStartTime(currentItem.dateInMs);
            endTime = DateUtils.getTodayEndTime(currentItem.dateInMs);
            //Logger.d(TAG, "updateClusterTime - preTime: " + preTime + " endTime: " + endTime);
        } catch (Exception e) {
            Logger.d(TAG, "updateClusterTime - err: " + e.getMessage());
        }
    }
...
class Cluster {
    ...
    public String generateCaption(Context context) {
        ...
        //String caption;
        //String minDay = DateUtils.FORMAT_DAY_OF_MONTH_2.format(minTimestamp).toString();
        //String maxDay = DateUtils.FORMAT_DAY_OF_MONTH_2.format(maxTimestamp).toString();
        ...
        //Logger.d(TAG, "minDay: " + minDay + ", maxDay: " + maxDay);

        Resources res = context.getResources();
        StringBuilder stringBuilder = new StringBuilder();

        if (DateUtils.isToday(minTimestamp)) {
            stringBuilder.append(res.getString(R.string.today));
        } else if (DateUtils.isYesterday(minTimestamp)) {
            stringBuilder.append(res.getString(R.string.yesterday));
        } else if (DateUtils.isYear(minTimestamp)) {
            stringBuilder.append(DateUtils.FORMAT_ONLY_MONTH_2.format(minTimestamp).toString());
        } else {
            stringBuilder.append(DateUtils.FORMAT_DAY_OF_MONTH_2.format(minTimestamp).toString());
        }
        stringBuilder.append(", " + DateUtils.getWeek(context, minTimestamp));
