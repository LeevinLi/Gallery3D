...
public class CategoryAdapter extends ArrayAdapter<Action> {
    ...
    public void initializeSelection(int category) {
        mCategory = category;
        mSelectedPosition = -1;
        if (category == FilterShowActivity.LOOKS) {
            mSelectedPosition = 0;
            mAddButtonText = getContext().getString(R.string.filtershow_add_button_looks);
        }
        if (category == FilterShowActivity.BORDERS) {
            mSelectedPosition = 0;
        }
        if (category == FilterShowActivity.VERSIONS) {
    ...
    public void remove(Action action) {
        if (!(mCategory == FilterShowActivity.VERSIONS
                || mCategory == FilterShowActivity.LOOKS)) {
            return;
        }
        super.remove(action);
        FilterShowActivity activity = (FilterShowActivity) getContext();
        if (mCategory == FilterShowActivity.LOOKS) {
            activity.removeLook(action);
        } else if (mCategory == FilterShowActivity.VERSIONS) {
    ...
    public void reflectImagePreset(ImagePreset preset) {
        ...
        } else if (mCategory == FilterShowActivity.BORDERS) {
