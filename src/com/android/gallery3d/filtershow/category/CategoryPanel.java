...
public class CategoryPanel extends Fragment implements View.OnClickListener {
    ...
    private int mCurrentAdapter = FilterShowActivity.LOOKS;
    ...
    public void loadAdapter(int adapter) {
        ...
        switch (adapter) {
            case FilterShowActivity.LOOKS: {
                mAdapter = activity.getCategoryLooksAdapter();
                if (mAdapter != null) {
                    mAdapter.initializeSelection(FilterShowActivity.LOOKS);
                }
                activity.updateCategories();
                break;
            }
            case FilterShowActivity.BORDERS: {
                mAdapter = activity.getCategoryBordersAdapter();
                if (mAdapter != null) {
                    mAdapter.initializeSelection(FilterShowActivity.BORDERS);
                }
                activity.updateCategories();
                break;
            }
            case FilterShowActivity.GEOMETRY: {
                mAdapter = activity.getCategoryGeometryAdapter();
                if (mAdapter != null) {
                    mAdapter.initializeSelection(FilterShowActivity.GEOMETRY);
                }
                break;
            }
            case FilterShowActivity.FILTERS: {
                mAdapter = activity.getCategoryFiltersAdapter();
                if (mAdapter != null) {
                    mAdapter.initializeSelection(FilterShowActivity.FILTERS);
                }
                break;
            }
            case FilterShowActivity.VERSIONS: {
                mAdapter = activity.getCategoryVersionsAdapter();
                if (mAdapter != null) {
                    mAdapter.initializeSelection(FilterShowActivity.VERSIONS);
                }
                break;
            }
        }
