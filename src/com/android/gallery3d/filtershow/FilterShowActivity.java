...
import com.android.gallery3d.filtershow.category.CategoryPanel;
...
import com.android.gallery3d.filtershow.ui.EffectlistViewAdapter;
...
public class FilterShowActivity extends FragmentActivity implements OnItemClickListener,
        ...
        DialogInterface.OnDismissListener, PopupMenu.OnDismissListener, OnClickListener {
    ...
    private int mCurrentPanel = LOOKS;
    ...
    private EffectListView mEffectListView;
    private boolean isportrait;
    private EffectlistViewAdapter mEffectAdapter;
    ...
    public void loadMainPanel() {
        ...
        panel.setAdapter(mCurrentPanel);
        FragmentTransaction transaction = getSupportFragmentManager().beginTransaction();
        transaction.replace(R.id.main_panel_container, panel, CategoryPanel.FRAGMENT_TAG);
        ...
        if (isportrait) {
            ...
            mFilterClose.setVisibility(View.GONE);
    ...
    public void loadEditorPanel(FilterRepresentation representation,
        ...
        Runnable showEditor = new Runnable() {
            ...
            public void run() {
                ...
                FragmentTransaction transaction = getSupportFragmentManager().beginTransaction();
                transaction.remove(getSupportFragmentManager().findFragmentByTag(CategoryPanel.FRAGMENT_TAG));
                transaction.replace(R.id.main_panel_container, panel, CategoryPanel.FRAGMENT_TAG);
                transaction.commit();
            }
        };
        Fragment category = getSupportFragmentManager().findFragmentByTag(CategoryPanel.FRAGMENT_TAG);
        boolean doAnimation = false;
        if (mShowingImageStatePanel
                && getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT) {
            doAnimation = true;
        }
        if (doAnimation && category != null && category instanceof CategoryPanel) {
            CategoryPanel Panel = (CategoryPanel) category;
            View container = findViewById(R.id.main_panel_container);
            View bottom = findViewById(R.id.bottom_panel);
            int panelHeight = container.getHeight() + bottom.getHeight();
            ViewPropertyAnimator anim = Panel.getView().animate();
    ...
    private void loadXML() {
        ...
        looksButton = (ImageButton) findViewById(R.id.fxButton);
        looksButton.setOnClickListener(this);
        bordersButton = (ImageButton) findViewById(R.id.borderButton);
        bordersButton.setOnClickListener(this);
        geometryButton = (ImageButton) findViewById(R.id.geometryButton);
        geometryButton.setOnClickListener(this);
        filtersButton = (ImageButton) findViewById(R.id.colorsButton);
        filtersButton.setOnClickListener(this);
        drawingButton = (ImageButton) findViewById(R.id.drawingButton);
        drawingButton.setOnClickListener(this);
        if (isportrait) {
            mEffectListView = (EffectListView) findViewById(R.id.effect_titles);
            String[] titles = {getResources().getString(R.string.crop),
                    getResources().getString(R.string.rotate),
                    getResources().getString(R.string.mirror)};
            mEffectAdapter = new EffectlistViewAdapter(getBaseContext(), titles);
            mEffectListView.setAdapter(mEffectAdapter);
        }

        mActionBar = getActionBar();
        mActionBar.setDisplayOptions(ActionBar.DISPLAY_SHOW_CUSTOM);
        mActionBar.setCustomView(R.layout.filtershow_actionbar_view);

        mSaveButton = mActionBar.getCustomView().findViewById(R.id.filtershow_actionbar_save);
        mSaveButton.setAlpha(0.5f);
        mSaveButton.setOnClickListener(this);
        mUndoButton = mActionBar.getCustomView().findViewById(R.id.filtershow_actionbar_undo);
        mUndoButton.setAlpha(0.5f);
        mUndoButton.setOnClickListener(this);
        mRedoButton = mActionBar.getCustomView().findViewById(R.id.filtershow_actionbar_redo);
        mRedoButton.setAlpha(0.5f);
        mRedoButton.setOnClickListener(this);
        mExitButton = mActionBar.getCustomView().findViewById(R.id.filtershow_actionbar_exit);
        mExitButton.setOnClickListener(this);
        if (!isportrait) {
            mFilterClose = (Button) mActionBar.getCustomView().findViewById(R.id.filtershow_actionbar_close);
            mFilterClose.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (EditorCurves.ID == mCurrentEditor.getID()) {
                        mCurrentEditor.getImageShow().resetParameter();
                        loadMainPanel();
                    } else {
                        resetHistory();
                    }
            ...
            mFilterCheck = (Button) mActionBar.getCustomView().findViewById(R.id.filtershow_actionbar_check);
            mFilterCheck.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    loadMainPanel();
                }
        ...
        mMasterImage.getHistory().setMenuItems(mUndoButton, mRedoButton, null);
    ...
    public void showRepresentation(FilterRepresentation representation) {
        ...
        if (mCurrentPanel == FILTERS) {
            if (isportrait) {
                ...
                mFilterClose.setVisibility(View.VISIBLE);
        ...
        if ((representation.getEditorId() != EditorMirror.ID) && (representation.getEditorId() != EditorRotate.ID)) {
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.filtershow_actionbar_save:
                saveImage();
                break;
            case R.id.filtershow_actionbar_undo:
                doUndo();
                break;
            case R.id.filtershow_actionbar_redo:
                doRedo();
                break;
            case R.id.filtershow_actionbar_exit:
                exitImage();
                break;
            case R.id.fxButton:
                showPanel(LOOKS);
                break;
            case R.id.borderButton:
                showPanel(BORDERS);
                break;
            case R.id.geometryButton:
                showPanel(GEOMETRY);
                break;
            case R.id.colorsButton:
                showPanel(FILTERS);
                break;
    ...
    private void setCategoryFragment(CategoryPanel category, boolean fromRight) {
        ...
        transaction.replace(R.id.main_panel_container, category, CategoryPanel.FRAGMENT_TAG);
        transaction.commitAllowingStateLoss();
    }
    ...
    public void loadCategoryGeometryPanel() {
        ...
        selection(mCurrentSelected, true);
        final FilterCropRepresentation cropRepresentation =
                (FilterCropRepresentation) mCategoryGeometryAdapter.getItem(0).getRepresentation();
        final FilterStraightenRepresentation straightenRepresentation =
                (FilterStraightenRepresentation) mCategoryGeometryAdapter.getItem(1).getRepresentation();
        final FilterMirrorRepresentation mirrorRepresentation =
                (FilterMirrorRepresentation) mCategoryGeometryAdapter.getItem(3).getRepresentation();
        if (mEffectListView != null) {
            mEffectListView.setVisibility(View.VISIBLE);
            mEffectAdapter.selected(0);
            mEffectAdapter.notifyDataSetChanged();
            mEffectListView.setOnItemClickListener(new OnItemClickListener() {
                @Override
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    mEffectAdapter.selected(position);
                    mEffectAdapter.notifyDataSetChanged();
                    switch (position) {
                        case 0:
                            showRepresentation(cropRepresentation);
                            break;
                        case 1:
                            showRepresentation(straightenRepresentation);
                            break;
                        case 2:
                            loadEditorPanel(mirrorRepresentation, mEditorPlaceHolder.showEditor(mirrorRepresentation.getEditorId()));
                            break;
                    }
                }
            });
    ...
    public void loadCategoryDrawingPanel() {
        if (mCurrentSelected == DRAWING) {
            return;
        }
        selection(mCurrentSelected, false);
        mCurrentSelected = DRAWING;
        selection(mCurrentSelected, true);
        if (mEffectListView != null) {
            mEffectListView.setVisibility(View.GONE);
        }
    ...
    private void selection(int position, boolean value) {
        ...
        switch (position) {
            ...
            case DRAWING: {
                drawingButton.setSelected(value);
                break;
            }
        }
    }
    ...
    public void enableSave(boolean enable) {
        if (mSaveButton != null) {
            ...
            mSaveButton.setAlpha(enable ? 1.0f : 0.5f);
        }
    }
