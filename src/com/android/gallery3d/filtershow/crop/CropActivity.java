...
public class CropActivity extends Activity {
    ...
    private void startLoadBitmap(Uri uri) {
        if (uri != null) {
            Logger.d(TAG, "startLoadBitmap - uri: " + uri.toString());
            enableSave(false);
    ...
    private void doneLoadBitmap(Bitmap bitmap, RectF bounds, int orientation) {
        ...
        if (bitmap != null && bitmap.getWidth() != 0 && bitmap.getHeight() != 0) {
            ...
            Logger.w(TAG, "could not load image for cropping");
            cannotLoadImage();
            setResult(RESULT_CANCELED, new Intent());
            done();
        }
    ...
    private void startBitmapIO(int flags, Bitmap currentBitmap, Uri sourceUri, Uri destUri,
                               RectF cropBounds, RectF photoBounds, RectF currentBitmapBounds, String format, int rotation) {
    ...
    private RectF getBitmapCrop(RectF imageBounds) {
        ...
        if (crop == null || photo == null) {
            Logger.w(TAG, "could not get crop");
            return null;
        }
    ...
    private class BitmapIOTask extends AsyncTask<Bitmap, Void, Boolean> {
        ...
        public BitmapIOTask(Uri sourceUri, Uri destUri, String outputFormat, int flags,
            ...
            if ((flags & DO_EXTRA_OUTPUT) != 0) {
                if (mOutUri == null) {
                    Logger.w(TAG, "cannot write file, no output URI given");
                } else {
                    try {
                        mOutStream = getContentResolver().openOutputStream(mOutUri);
                    } catch (FileNotFoundException e) {
                        Logger.w(TAG, "cannot write file: " + mOutUri.toString(), e);
                    }
                }
            }
        ...
        private void regenerateInputStream() {
            if (mInUri == null) {
                Logger.w(TAG, "cannot read original file, no input URI given");
            } else {
                Utils.closeSilently(mInStream);
                try {
                    mInStream = getContentResolver().openInputStream(mInUri);
                } catch (FileNotFoundException e) {
                    Logger.w(TAG, "cannot read file: " + mInUri.toString(), e);
                }
            }
        }
        ...
        protected Boolean doInBackground(Bitmap... params) {
            ...
            if ((mFlags & DO_RETURN_DATA) != 0) {
                ...
                if (ret == null) {
                    Logger.w(TAG, "could not downsample bitmap to return in data");
                    failure = true;
                } else {
                    if (mRotation > 0) {
                        Matrix m = new Matrix();
            ...
            if ((mFlags & (DO_EXTRA_OUTPUT | DO_SET_WALLPAPER)) != 0 && mInStream != null) {
                ...
                RectF trueCrop = CropMath.getScaledCropBounds(mCrop, mPhoto, mOrig);
                if (trueCrop == null) {
                    Logger.w(TAG, "cannot find crop for full size image");
                    failure = true;
                    return false;
                }
                ...
                try {
                    ...
                    Logger.w(TAG, "cannot open region decoder for file: " + mInUri.toString(), e);
                }
                ...
                if (crop == null) {
                    Logger.w(TAG, "cannot decode file: " + mInUri.toString());
                    failure = true;
                    return false;
                }
                if (mOutputX > 0 && mOutputY > 0) {
                ...
                if (mFlags == DO_EXTRA_OUTPUT) {
                    if (mOutStream == null
                        ...
                        Logger.w(TAG, "failed to compress bitmap to file: " + mOutUri.toString());
                        failure = true;
                    } else {
                    ...
                    if (crop.compress(cf, DEFAULT_COMPRESS_QUALITY, tmpOut)) {
                        ...
                        if ((mFlags & DO_SET_WALLPAPER) != 0 && mWPManager != null) {
                            if (mWPManager == null) {
                                Logger.w(TAG, "no wallpaper manager");
                                failure = true;
                            } else {
                                try {
                                    mWPManager.setStream(new ByteArrayInputStream(tmpOut.toByteArray()));
                                } catch (IOException e) {
                                    Logger.w(TAG, "cannot write stream to wallpaper", e);
                                    failure = true;
                                }
                            }
                        }
