...
public class DecoderThread extends Thread {
    ...
    public DecoderThread(byte[] data, DecoderListener act) {
        mGifData = data;
        mDecoderListener = act;
    }

    public DecoderThread(InputStream is, DecoderListener act) {
        mIS = is;
        mDecoderListener = act;
    }
        ...
        if (mIS != null) {
            ...
            if (!err()) {
                ...
                if (mFrameCount < 0) {
                    ...
                    mDecoderListener.onDecode(false, -1);
                } else {
                    mStatus = STATUS_FINISH;
                    mDecoderListener.onDecode(true, -1);
                }
            }
            try {
                mIS.close();
    ...
    private void readImage() {
        ...
        try {
            ...
            resetFrame();
            mDecoderListener.onDecode(true, mFrameCount);
        } catch (OutOfMemoryError e) {
            Logger.e("DecoderThread", ">>> log  : " + e.toString());
            e.printStackTrace();
        }
    }
