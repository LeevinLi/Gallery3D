...
public class MenuExecutor {
    ...
    public static void updateMenuOperation(Menu menu, int supported) {
        ...
        boolean supportRename = (supported & MediaObject.SUPPORT_RENAME) != 0;
        ...
        setMenuItemVisible(menu, R.id.action_share_panorama, true);
        ...
        setMenuItemVisible(menu, R.id.action_rename, supportRename);
    }
    ...
    private void onProgressComplete(int result, ProgressListener listener) {
        Message msg = mHandler.obtainMessage(MSG_TASK_COMPLETE, result, 0, listener);
    ...
    public void startAction(int action, int title, ProgressListener listener, boolean waitOnStop, boolean showDialog) {
        ...
        try {
            if (R.id.action_delete == action
                    && (ids.contains(Path.fromString(LocalPanoramasAlbumSet.LOCAL_ALL_PANORAMAS_PATH))
                    || ids.contains(Path.fromString(LocalVideoAlbumSet.LOCAL_ALL_VIDEO_PATH)))) {
                Logger.w(TAG, "get panoramas and video media items 'ids' with action 'delete'");
                ids.addAll(mSelectionManager.getSelected(true));
            }
        } catch (Exception e) {
            Logger.w(TAG, "get panoramas and video album ids err: " + e.getMessage());
        }
    ...
    private boolean execute(DataManager manager, JobContext jc, int cmd, Path path) {
        boolean result = true;
        Logger.v(TAG, "Execute cmd: " + cmd + " for " + path);
        ...
        Logger.v(TAG, "It takes " + (System.currentTimeMillis() - startTime) + " ms to execute cmd for " + path);
        return result;
    }
    ...
    private class MediaOperation implements Job<Void> {
        ...
        public Void run(JobContext jc) {
            ...
            boolean isContainEmptyAlbum = false;
            try {
                ...
                for (Path id : mItems) {
                    ...
                    //delete empty album
                    if (id.getParent().equalsIgnoreCase("/local/empty")) {
                        isContainEmptyAlbum = true;
                        String bucketId = id.getSuffix();
                        Logger.d(TAG, "delete empty album bucketId: " + bucketId);
                        mActivity.getAlbumsManager().delete(Integer.parseInt(bucketId));
                        continue;
                    }

                    Logger.d(TAG, "delete album");
                ...
                //sync data
                if (isContainEmptyAlbum) {
                    MediaSetUtils.requestSync(mActivity.getAndroidContext());
                }
            } catch (Throwable th) {
                Logger.e(TAG, "failed to execute operation " + mOperation + " : " + th);
            } finally {
