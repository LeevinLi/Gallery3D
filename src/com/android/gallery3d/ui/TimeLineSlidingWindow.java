...
public class TimeLineSlidingWindow implements TimeLineDataLoader.DataListener {
    ...
    public TimeLineSlidingWindow(AbstractGalleryActivity activity,
            ...
            public void handleMessage(Message message) {
                switch (message.what) {
                    case MSG_UPDATE_ENTRY:
                        //Logger.d(TAG,"mHandler - ENTRY");
                        ((ThumbnailLoader) message.obj).updateEntry();
                        break;
                    case MSG_UPDATE_LABEL:
                        //Logger.d(TAG,"mHandler - LABEL");
                        ((TimeLabelLoader) message.obj).updateEntry();
                        break;
                }
            }
        };
        ...
        mLabelMaker = new TimeLineLabelMaker(activity.getAndroidContext(), labelSpec);
    }
    ...
    public void setActiveWindow(int start, int end) {
        ...
        //Logger.d(TAG,"setActiveWindow - start: " + start +" end: " + end);
    ...
    private boolean requestSlotImage(int slotIndex) {
        ...
        //Logger.d(TAG,"requestSlotImage - slotIndex: " + slotIndex);
        //start load content
        if (entry.contentLoader != null) entry.contentLoader.startLoad();
        //start load label
    ...
    private void cancelSlotImage(int slotIndex) {
        ...
        if (item.labelLoader != null) item.labelLoader.cancelLoad();
    }

    private void freeSlotContent(int slotIndex) {
        //Logger.d(TAG,"freeSlotContent- slotIndex: " + slotIndex);
        AlbumEntry data[] = mData;
    ...
    private void prepareSlotContent(int slotIndex) {
        //Logger.d(TAG,"prepareSlotContent - slotIndex: " + slotIndex);
        ...
        if (entry.item != null) {
            entry.labelLoader = new TimeLabelLoader(slotIndex, entry.item.getDuration());
        }
        mData[slotIndex % mData.length] = entry;
    }

    private void updateAllTimelineTitleContent(boolean needRequest) {
        //Logger.d(TAG,"updateAllTimelineTitleContent - needRequest: " + needRequest);
    ...
    public void onSizeChanged() {
        ...
        Logger.d(TAG, "onSizeChanged - size: " + size);
        if (mSize != size) {
            mSize = size;
            ...
            //init time line title data for frame title
            if (mTitleList != null) {
                mTitleList.clear();
                mTitleList = null;
            }
            int titlesSize = mSource.getTimeLineTitlesCount();
            mTitleList = new ArrayList<>(titlesSize);
            for (int i = 0; i < titlesSize; i++) {
                MediaSet sets = mSource.getSubMediaSet(i);
                String titleName = sets.getName();
                if (!mTitleList.contains(titleName)) {
                    //Logger.d(TAG, i + " title item name: " + titleName);
                    mTitleList.add(sets.getName());
                }
            }
            //Logger.d(TAG, "title list size: " + mTitleList.size() + " titlesSize: " + titlesSize);
        }
    }

    /**
     * get title name by index
     *
     * @param index
     * @return
     */
    public String getTileName(int index) {
        String name = null;
        if (mTitleList == null) return name;
        int size = mTitleList.size();
        if (index < 0 || index >= size) return name;
        name = mTitleList.get(index);
        //Logger.d(TAG, "size: " + size + " index: " + index + " name: " + name);
        return name;
    }

    @Override
    public void onContentChanged(int index) {
        //Logger.d(TAG,"onContentChanged - index: " + index);
    ...
    public void onSlotSizeChanged(int width, int height) {
        //Logger.d(TAG, "onSlotSizeChanged()");
        if (mSlotWidth == width) {
    ...
    private class ThumbnailLoader extends BitmapLoader {
        ...
        protected Future<Bitmap> submitBitmapTask(FutureListener<Bitmap> l) {
            if (mItem.getMediaType() != MediaObject.MEDIA_TYPE_TIMELINE_TITLE) {
                ...
                return mThreadPool.submit(((TimeLineTitleMediaItem) mItem).requestTitle(MediaItem.TYPE_MICROTHUMBNAIL, mTitleMaker), this);
            }
            return null;
        }
    ...
    private class TimeLabelLoader extends BitmapLoader {
        private final int mSlotIndex;
        private final int mDuration;

        public TimeLabelLoader(int slotIndex, int duration) {
            mSlotIndex = slotIndex;
            mDuration = duration;
        }
        ...
        @Override
        protected Future<Bitmap> submitBitmapTask(FutureListener<Bitmap> l) {
            String sDuration = GalleryUtils.formatDuration(mActivity.getAndroidContext(), mDuration);
            return mThreadPool.submit(mLabelMaker.requestLabel(sDuration), l);
        }
