...
public class AlbumSelectionSlidingWindow implements AlbumSelectionDataLoader.DataListener {
    private static final String TAG = "AlbumSelectionSlidingWindow";
    private static final int MSG_UPDATE_ALBUM_ENTRY = 1;
    //public static final String KEY_ALBUM = "AlbumSet";
    private final AlbumSelectionDataLoader mSource;
    private final AlbumSelectionEntry mData[];
    private final SynchronizedHandler mHandler;
    private final ThreadPool mThreadPool;
    private final AlbumSelectionLabelMaker mLabelMaker;
    ...
    public AlbumSelectionSlidingWindow(AbstractGalleryActivity activity, AlbumSelectionDataLoader source,
                                       AlbumSelectionSlotRenderer.LabelSpec labelSpec, int cacheSize) {
        source.setModelListener(this);
        mSource = source;
        mData = new AlbumSelectionEntry[cacheSize];
        mSize = source.size();
        mThreadPool = activity.getThreadPool();

        mLabelMaker = new AlbumSelectionLabelMaker(activity.getAndroidContext(), labelSpec);
    ...
    public AlbumSelectionEntry get(int slotIndex) {
    ...
    public void setActiveWindow(int start, int end) {
        ...
        AlbumSelectionEntry data[] = mData;
        mActiveStart = start;
        mActiveEnd = end;
    ...
    private void requestImagesInSlot(int slotIndex) {
        ...
        AlbumSelectionEntry entry = mData[slotIndex % mData.length];
    ...
    private boolean isLabelChanged(AlbumSelectionEntry entry, String title, int totalCount, int sourceType) {
        return !Utils.equals(entry.title, title) || entry.totalCount != totalCount || entry.sourceType != sourceType;
    }

    private void updateAlbumSetEntry(AlbumSelectionEntry entry, int slotIndex) {
    ...
    private void prepareSlotContent(int slotIndex) {
        AlbumSelectionEntry entry = new AlbumSelectionEntry();
    ...
    private void uploadBackgroundTextureInSlot(int index) {
        ...
        AlbumSelectionEntry entry = mData[index % mData.length];
    ...
    private void updateTextureUploadQueue() {
        ...
        for (int i = mActiveStart, n = mActiveEnd; i < n; ++i) {
            AlbumSelectionEntry entry = mData[i % mData.length];
            if (entry.bitmapTexture != null) {
                //mContentUploader.addTexture(entry.bitmapTexture);
                TiledTexture[] bitmapTextures = entry.bitmapTexture;
                if (bitmapTextures != null && bitmapTextures.length > 0) {
    ...
    public static class AlbumSelectionEntry {
    ...
    private class AlbumCoverLoader extends BitmapLoader implements EntryUpdater {
        ...
        public void updateEntry() {
            ...
            AlbumSelectionEntry entry = mData[mSlotIndex % mData.length];
