...
public class PhotoView extends GLView {
    ...
    private void updateCameraRect() {
        ...
        Logger.d(TAG, "compensation = " + mCompensation
    ...
    class FullPicture implements Picture {
        ...
        private void drawTileView(GLCanvas canvas, Rect r) {
            ...
            if (mIsVideo || mIsGif) drawVideoPlayIcon(canvas, s);
...
//                if (mListener != null) {
//                    mListener.onScroll(dx, dy, totalX, totalY);
//                }
            }
            return true;
        }
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            //response back event
            onBack(e1, e2, velocityX, velocityY);
        ...
        private boolean onBack(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            int vx = (int) (velocityX + 0.5f);
            int vy = (int) (velocityY + 0.5f);
            int escapeVelocity = GalleryUtils.dpToPixel(70);    // 105/sec
            int minDistance = GalleryUtils.dpToPixel(40);   //60
            int escapeDistance = GalleryUtils.dpToPixel(160);   //240

            boolean isBack = mPositionController.isAtMinimalScale()
                    //&& Math.abs(vx) < escapeVelocity
                    && Math.abs(e2.getX() - e1.getX()) < minDistance
                    //&& (Math.abs(vy) > escapeVelocity)
                    && Math.abs(e2.getY() - e1.getY()) > escapeDistance;

            /*
            Logger.d(TAG, "onBack - vx: " + vx + " vy: " + vy +
                    " dy: " + Math.abs(e1.getY() - e2.getY()) + " dx: " + Math.abs(e1.getX() - e2.getX()));
            Logger.d(TAG, "isAtMinimalScale: " + mPositionController.isAtMinimalScale() + " escapeVelocity: " +
                    escapeVelocity + " minDistance: " + minDistance + " escapeDistance: " + escapeDistance);
            */
            if (isBack) {
                Logger.d(TAG, "onBack - fling up and down");
                if (mListener != null) {
                    mListener.onBack();
                }
                return true;
