...
import com.android.gallery3d.app.AlbumSettingDataLoader;
...
public class AlbumSettingSlidingWindow implements AlbumSettingDataLoader.DataListener {

    private static final String TAG = "AlbumSettingSlidingWindow";

    private static final int MSG_UPDATE_ALBUM_ENTRY = 1;
    //public static final String KEY_ALBUM = "AlbumSet";

    private final AlbumSettingDataLoader mSource;
    private final AlbumSettingEntry mData[];
    private final SynchronizedHandler mHandler;
    private final ThreadPool mThreadPool;
    private final AlbumSettingLabelMaker mLabelMaker;
    ...
    public AlbumSettingSlidingWindow(AbstractGalleryActivity activity, AlbumSettingDataLoader source,
                                     AlbumSettingSlotRenderer.LabelSpec labelSpec, int cacheSize) {
        source.setModelListener(this);
        mSource = source;
        mData = new AlbumSettingEntry[cacheSize];
        mSize = source.size();
        mThreadPool = activity.getThreadPool();

        mLabelMaker = new AlbumSettingLabelMaker(activity.getAndroidContext(), labelSpec);
    ...
    public AlbumSettingEntry get(int slotIndex) {
    ...
    private void requestImagesInSlot(int slotIndex) {
        ...
        AlbumSettingEntry entry = mData[slotIndex % mData.length];
        //if (entry.coverLoader != null) entry.coverLoader.startLoad();
        if (entry.labelLoader != null) entry.labelLoader.startLoad();
        BitmapLoader[] coverLoaders = entry.coverLoader;
        if (coverLoaders != null && coverLoaders.length > 0) {
            for (BitmapLoader loader : coverLoaders) {
    ...
    private void freeSlotContent(int slotIndex) {
        ...
        for (BitmapLoader loader : entry.coverLoader) {
            if (loader != null) loader.recycle();
        }
        for (TiledTexture t : entry.bitmapTexture) {
            if (t != null) t.recycle();
        }
        mData[slotIndex % mData.length] = null;
    }

    private boolean isLabelChanged(AlbumSettingEntry entry, String title, int totalCount, int sourceType) {
        return !Utils.equals(entry.title, title) || entry.totalCount != totalCount || entry.sourceType != sourceType;
    }

    private void updateAlbumSetEntry(AlbumSettingEntry entry, int slotIndex) {
        ...
        entry.status = (album == null) ? MediaSet.ALBUM_FLAG_HIDDEN : album.getStatus();
        ...
        entry.coverDataVersion = new long[3];
        entry.coverLoader = new BitmapLoader[3];
        entry.content = new Texture[3];
        entry.bitmapTexture = new TiledTexture[3];
        if (cover == null) return;
        for (int i = 0; i < cover.length; i++) {
            if (getDataVersion(cover[i]) != entry.coverDataVersion[i]) {
                entry.coverDataVersion[i] = getDataVersion(cover[i]);
                entry.rotation = (cover[i] == null) ? 0 : cover[i].getRotation();
                if (entry.coverLoader[i] != null) {
                    entry.coverLoader[i].recycle();
                    entry.coverLoader[i] = null;
                    entry.bitmapTexture[i] = null;
                    entry.content[i] = null;
                }
                if (cover[i] != null) {
                    entry.coverLoader[i] = new AlbumCoverLoader(slotIndex, cover[i], i);
                }
            }
        }
    }
    ...
    private void prepareSlotContent(int slotIndex) {
        AlbumSettingEntry entry = new AlbumSettingEntry();
    ...
    private void uploadBackgroundTextureInSlot(int index) {
        ...
        AlbumSettingEntry entry = mData[index % mData.length];
        if (entry.bitmapTexture != null) {
            //mContentUploader.addTexture(entry.bitmapTexture);
            TiledTexture[] bitmapTextures = entry.bitmapTexture;
            if (bitmapTextures != null && bitmapTextures.length > 0) {
                for (TiledTexture t : bitmapTextures) {
                    if (t != null) {
                        mContentUploader.addTexture(t);
                    }
                }
            }
        }
    ...
    private void updateTextureUploadQueue() {
        ...
        for (int i = mActiveStart, n = mActiveEnd; i < n; ++i) {
            AlbumSettingEntry entry = mData[i % mData.length];
            if (entry.bitmapTexture != null) {
                //mContentUploader.addTexture(entry.bitmapTexture);
                TiledTexture[] bitmapTextures = entry.bitmapTexture;
                if (bitmapTextures != null && bitmapTextures.length > 0) {
    ...
    private void updateAllImageRequests() {
        ...
        for (int i = mActiveStart, n = mActiveEnd; i < n; ++i) {
            ...
            BitmapLoader[] coverLoaders = entry.coverLoader;
            if (coverLoaders != null && coverLoaders.length > 0) {
                for (BitmapLoader coverLoader : coverLoaders) {
                    if (startLoadBitmap(coverLoader)) ++mActiveRequestCount;
                }
            }
        }
    ...
    public void onContentChanged(int index) {
        ...
        if (index < mContentStart || index >= mContentEnd) {
            Logger.w(TAG, String.format("invalid update: %s is outside (%s, %s)", index, mContentStart, mContentEnd));
            return;
        }
    ...
    public BitmapTexture getLoadingTexture() {
        if (mLoadingLabel == null) {
            Bitmap bitmap = mLabelMaker.requestLabel(mLoadingText, "",
                    DataSourceType.TYPE_NOT_CATEGORIZED).run(ThreadPool.JOB_CONTEXT_STUB);
    ...
    public static class AlbumSettingEntry {
    ...
    private class AlbumCoverLoader extends BitmapLoader implements EntryUpdater {
        ...
        public AlbumCoverLoader(int slotIndex, MediaItem item, int itemIndex) {
        ...
        public void updateEntry() {
            ...
            AlbumSettingEntry entry = mData[mSlotIndex % mData.length];
            TiledTexture texture = new TiledTexture(bitmap);
            entry.bitmapTexture[mItemIndex] = texture;
            entry.content[mItemIndex] = texture;
