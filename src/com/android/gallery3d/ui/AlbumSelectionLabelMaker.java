...
public class AlbumSelectionLabelMaker {
    private static final String TAG = "AlbumSelectionLabelMaker";
    private static final int BORDER_SIZE = 0;

    private final AlbumSelectionSlotRenderer.LabelSpec mSpec;
    ...
    public AlbumSelectionLabelMaker(Context context, AlbumSelectionSlotRenderer.LabelSpec spec) {
        mContext = context;
        mSpec = spec;
    ...
    private class AlbumLabelJob implements ThreadPool.Job<Bitmap> {
        ...
        public Bitmap run(JobContext jc) {
            AlbumSelectionSlotRenderer.LabelSpec spec = mSpec;
            ...
            int x = 0;
            if (icon != null) {
                x = spec.leftMargin + spec.iconSize;
            } else {
                x = spec.leftMargin;
            }
            // TODO: is the offset relevant in new reskin?
            // int y = s.titleOffset;
            int y = (spec.labelBackgroundHeight - spec.titleFontSize) / 2;
            drawText(canvas, x, y, title, labelWidth - spec.leftMargin - x - spec.titleRightMargin, mTitlePaint);

            // draw count
            if (jc.isCancelled()) return null;
            x = labelWidth - spec.titleRightMargin;
            y = (spec.labelBackgroundHeight - spec.countFontSize) / 2;
