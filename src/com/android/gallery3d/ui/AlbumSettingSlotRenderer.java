...
import com.android.gallery3d.ui.AlbumSettingSlidingWindow.AlbumSettingEntry;
import com.android.gallery3d.util.GalleryUtils;

public class AlbumSettingSlotRenderer extends AbstractSlotRenderer {
    @SuppressWarnings("unused")
    private static final String TAG = "AlbumSettingSlotRenderer";
    private static final int CACHE_SIZE = 96;
    protected final LabelSpec mLabelSpec;
    private final int mPlaceholderColor;
    private final ColorTexture mWaitLoadingTexture;
    //private final ResourceTexture mCameraOverlay;
    private final ResourceTexture mLockOverlay;
    private final ResourceTexture mUnLockOverlay;
    private final AbstractGalleryActivity mActivity;
    private final SelectionManager mSelectionManager;
    protected AlbumSettingSlidingWindow mDataWindow;
    ...
    public AlbumSettingSlotRenderer(AbstractGalleryActivity activity, SelectionManager selectionManager,
        ...
        mLockOverlay = new ResourceTexture(activity, R.drawable.ic_btn_lock_rest);
        mUnLockOverlay = new ResourceTexture(activity, R.drawable.ic_btn_unlock_rest);
    }
    ...
    public void setModel(AlbumSettingDataLoader model) {
        if (mDataWindow != null) {
        ...
        if (model != null) {
            mDataWindow = new AlbumSettingSlidingWindow(mActivity, model, mLabelSpec, CACHE_SIZE);
    ...
    public int renderSlot(GLCanvas canvas, int index, int pass, int width, int height) {
        AlbumSettingEntry entry = mDataWindow.get(index);
    ...
    protected int renderOverlay(GLCanvas canvas, int index, AlbumSettingEntry entry, int width, int height) {
        int renderRequestFlags = 0;
        int uncoveredHeight = height - mLabelSpec.labelBackgroundHeight;
        int dim = uncoveredHeight / 2;
        if (entry.album != null && entry.status == MediaSet.ALBUM_FLAG_HIDDEN) {
            drawSelectedBackground(canvas, width, height);
            mLockOverlay.draw(canvas, (width - dim) / 2, (uncoveredHeight - dim) / 2, dim, dim);
        } else {
            mUnLockOverlay.draw(canvas, (width - dim) / 2, (uncoveredHeight - dim) / 2, dim, dim);
        }
        return renderRequestFlags;
    }

    protected int renderContent(GLCanvas canvas, AlbumSettingEntry entry, int width, int height) {
        int renderRequestFlags = 0;
    ...
    protected int renderContent3D(GLCanvas canvas, AlbumSettingEntry entry, int width, int height) {
        int renderRequestFlags = 0;
        int oneSize = width;
    ...
    protected int renderContent(int textureIndex, boolean isAlpha, GLCanvas canvas, AlbumSettingEntry entry, int drawX, int drawY, int
            width, int height) {
        int renderRequestFlags = 0;
        if (entry == null) {
    ...
    protected int renderLabel(GLCanvas canvas, AlbumSettingEntry entry, int width, int height) {
        Texture content = checkLabelTexture(entry.labelTexture);
        if (content == null) {
            content = mWaitLoadingTexture;
        }
        int b = AlbumSettingLabelMaker.getBorderSize();
    ...
    private class MyCacheListener implements AlbumSettingSlidingWindow.Listener {
