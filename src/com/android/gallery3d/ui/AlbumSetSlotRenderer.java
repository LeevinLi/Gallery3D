...
public class AlbumSetSlotRenderer extends AbstractSlotRenderer {
    ...
    public void setModel(AlbumSetDataLoader model) {
        ...
        if (model != null) {
            mDataWindow = new AlbumSetSlidingWindow(mActivity, model, mLabelSpec, CACHE_SIZE);
    ...
    public int renderSlot(GLCanvas canvas, int index, int pass, int width, int height) {
        ...
        //Logger.d(TAG,"renderSlot - pass: " + pass +" width: " + width +" height: "+ height);
...
//        }
    ...
    protected int renderContent(GLCanvas canvas, AlbumSetEntry entry, int width, int height) {
        int renderRequestFlags = 0;
    ...
    protected int renderContent3D(GLCanvas canvas, AlbumSetEntry entry, int width, int height) {
        int renderRequestFlags = 0;
        int oneSize = width;
        ...
        int threeY = twoY - 5;
        if (entry == null && entry.coverItems == null || entry.coverItems.length <= 0) {
            Logger.d(TAG, "entry is null");
            //mBackOverlay.draw(canvas, oneCoordinate, oneCoordinate, oneSize, oneSize);
        } else {
            if (entry.coverItems.length == 1) {
    ...
    protected int renderContent(int textureIndex, boolean isAlpha, GLCanvas canvas, AlbumSetEntry entry, int drawX, int drawY, int width,
                                int height) {
        int renderRequestFlags = 0;
        if (entry == null) {
    ...
    protected int renderLabel(GLCanvas canvas, AlbumSetEntry entry, int width, int height) {
        Texture content = checkLabelTexture(entry.labelTexture);
        if (content == null) {
            content = mWaitLoadingTexture;
        }
        int b = AlbumSetLabelMaker.getBorderSize();
    ...
    private class MyCacheListener implements AlbumSetSlidingWindow.Listener {
        ...
        public void onSizeChanged(int size) {
            //Logger.d(TAG, "AlbumSetSlidingWindow.Listener - onSizeChanged size: " + size);
            mSlotView.setSlotCount(size);
        }

        @Override
        public void onContentChanged() {
            //Logger.d(TAG,"AlbumSetSlidingWindow.Listener - onContentChanged");
            mSlotView.invalidate();
        }
    }
}
