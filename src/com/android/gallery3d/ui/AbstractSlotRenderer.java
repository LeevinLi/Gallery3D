...
public abstract class AbstractSlotRenderer implements SlotView.SlotRenderer {
    ...
    private final ColorTexture mSelectedBackground;
    ...
    protected AbstractSlotRenderer(Context context) {
        ...
        mSelectedIcon = new ResourceTexture(context, R.drawable.ic_selected_on);

        int pressed_background = context.getResources().getColor(R.color.pressed_background);
        mSelectedBackground = new ColorTexture(pressed_background);
    }
    ...
    protected void drawContent(GLCanvas canvas, Texture content, int x, int y, int width, int height, int rotation) {
        canvas.save(GLCanvas.SAVE_FLAG_MATRIX);
        canvas.translate(0, 0, 20);
        if (content instanceof TiledTexture) {
            TiledTexture t = (TiledTexture) content;
            RectF source = getCropCenter(t);
            RectF target = new RectF();
            target.set(x, y, x + width, y + height);
            t.draw(canvas, source, target);
        } else {
    ...
    public RectF getCropCenter(Texture t) {
        RectF r = new RectF();
        r.set(0, 0, t.getWidth(), t.getHeight());
        int w = t.getWidth();
        int h = t.getHeight();
        if (w > h) {
            r.left = (w - h) / 2f;
    ...
    protected void drawSelectedOverlay(GLCanvas canvas, int width, int height) {
        mSelectedBackground.draw(canvas, 0, 0, width, height);

        int s = Math.min(width, height) / 4;
        mSelectedIcon.draw(canvas, width - s, 0, s, s);
    }

    protected void drawSelectedBackground(GLCanvas canvas, int width, int height) {
        mSelectedBackground.draw(canvas, 0, 0, width, height);
    }
