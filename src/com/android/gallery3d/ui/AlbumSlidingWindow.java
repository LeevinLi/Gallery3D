...
public class AlbumSlidingWindow implements AlbumDataLoader.DataListener {
    ...
    public AlbumSlidingWindow(AbstractGalleryActivity activity, AlbumDataLoader source,
                              AlbumSlotRenderer.LabelSpec labelSpec, int cacheSize) {
    ...
    private boolean requestSlotImage(int slotIndex) {
        ...
        return entry.contentLoader.isRequestInProgress() || entry.labelLoader.isRequestInProgress();
    ...
    private void cancelSlotImage(int slotIndex) {
        ...
        if (item.labelLoader != null) item.labelLoader.recycle();
    }
    ...
    public void onSlotSizeChanged(int width, int height) {
        ...
        for (int i = mContentStart, n = mContentEnd; i < n; ++i) {
            ...
            if (entry.item != null) {
                entry.labelLoader = new TimeLabelLoader(i, entry.item.getDuration());
            }
        }
        updateAllImageRequests();
        updateTextureUploadQueue();
