...
public class StateManager {
    ...
    public void startState(Class<? extends ActivityState> klass, Bundle data) {
        ...
        if (!mStack.isEmpty()) {
            ...
            top.transitionOnNextPause(top.getClass(), klass, StateTransitionAnimation.Transition.None);
    ...
    public void startStateForResult(Class<? extends ActivityState> klass, int requestCode, Bundle data) {
        Logger.v(TAG, "startStateForResult " + klass + ", " + requestCode);
        ActivityState state = null;
        try {
            state = klass.newInstance();
        } catch (Exception e) {
    ...
    void finishState(ActivityState state, boolean fireOnPause) {
        ...
        // If it is rejected, we won't close the last mGalleryViewPager.
        if (mStack.size() == 1) {
            ...
            if (!activity.isFinishing()) {
                Logger.w(TAG, "finish is rejected, keep the last state");
                return;
            }
            Logger.v(TAG, "no more state, finish activity");
        }

        Logger.v(TAG, "finishState " + state);
        if (state != mStack.peek().activityState) {
            if (state.isDestroyed()) {
                Logger.d(TAG, "The state is already destroyed");
                return;
            } else {
        ...
        if (mIsResumed && fireOnPause) {
            if (top != null) {
                state.transitionOnNextPause(state.getClass(), top.getClass(), StateTransitionAnimation.Transition.None);
            }
            state.onPause();
        }
        ...
        if (top != null) {
            UsageStatistics.onContentViewChanged(UsageStatistics.COMPONENT_GALLERY, top.getClass().getSimpleName());
        }
    }

    public void switchState(ActivityState oldState, Class<? extends ActivityState> klass, Bundle data) {
        Logger.v(TAG, "switchState " + oldState + ", " + klass);
    ...
    public void restoreFromState(Bundle inState) {
        ...
        for (Parcelable parcelable : list) {
            ...
            try {
                Logger.v(TAG, "restoreFromState " + klass);
    ...
    public void saveState(Bundle outState) {
        ...
        for (StateEntry entry : mStack) {
            ...
            Logger.v(TAG, "saveState " + entry.activityState.getClass());
            list[i++] = bundle;
        }
