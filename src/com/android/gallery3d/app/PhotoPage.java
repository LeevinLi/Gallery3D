...
public abstract class PhotoPage extends ActivityState implements
    ...
    public void onCreate(Bundle data, Bundle restoreState) {
        ...
        mIsFromCamera = data.getBoolean(KEY_FROM_CAMERA);
                ...
                switch (message.what) {
                    ...
                    case MSG_REFRESH_BOTTOM_CONTROLS: {
                        if (mCurrentPhoto == message.obj && mBottomControls != null) {
                            ...
                            Logger.d(TAG, "mHandler - mIsPanorama: " + mIsPanorama + " - 360: " + mIsPanorama360);
        ...
        mIsFromTimeLine = data.getBoolean(KEY_FROM_TIMELINE_SCREEN, false);

        boolean inCameraRoll = false;//data.getBoolean(KEY_IN_CAMERA_ROLL, false);
        ...
        if (mSetPathString != null) {
            ...
            if (mMediaSet != null && mIsFromTimeLine) {
                mMediaSet.setShowTimeTitle(false);
            }
            if (mMediaSet == null) {
                Logger.w(TAG, "failed to restore " + mSetPathString);
            }
            if (itemPath == null) {
        ...
        RelativeLayout galleryRoot = (RelativeLayout) ((Activity) mActivity).findViewById(mAppBridge != null ? R.id.content : R.id
                .gallery_root);
        if (galleryRoot != null) {
    ...
    public boolean canDisplayBottomControl(int control) {
        ...
        switch (control) {
            ...
            case R.id.photopage_bottom_control_background:
                return mShowBars
                        && ((mCurrentPhoto.getSupportedOperations() & MediaItem.SUPPORT_DELETE) != 0
                        || (mCurrentPhoto.getSupportedOperations() & MediaItem.SUPPORT_EDIT) != 0
                        || (mCurrentPhoto.getSupportedOperations() & MediaItem.SUPPORT_SHARE) != 0);
    ...
    public void onBottomControlClicked(int control) {
        switch (control) {
            case R.id.photopage_bottom_control_edit:
                Logger.d(TAG, "bottom_control_edit");
                launchPhotoEditor();
                return;
            case R.id.photopage_bottom_control_share:
                launchShare();
    ...
    private void overrideTransitionToEditor() {
        ((Activity) mActivity).overridePendingTransition(android.R.anim.fade_in, android.R.anim.fade_out);
    }
    ...
    private void launchShare() {
        ...
        intent.putExtra(Intent.EXTRA_STREAM, Uri.fromFile(new File(mCurrentPhoto.getFilePath())));
        intent.addFlags(Intent.FLAG_ACTIVITY_FORWARD_RESULT | Intent.FLAG_ACTIVITY_PREVIOUS_IS_TOP);
        mActivity.startActivity(Intent.createChooser(intent, mActivity.getResources().getString(R.string.share)));
    }
    ...
    private void updateMenuOperations() {
        ...
        if (BitmapUtils.isRotationSupported(mCurrentPhoto.getMimeType()) && !mReadOnlyView) {
            supportedOperations |= MediaObject.SUPPORT_ROTATE;
        }
        //if (mReadOnlyView) {
        //    supportedOperations = 0 ^ MediaObject.SUPPORT_INFO;
        //}
    ...
    private void refreshHidingMessage() {
        mHandler.removeMessages(MSG_HIDE_BARS);
        if (!mIsMenuVisible && !mPhotoView.getFilmMode() && mIsActive) {
    ...
    private void onUpPressed() {
        ...
        if (mReadOnlyView || mIsFromCamera) {
            super.onBackPressed();
            return;
        }
    ...
    protected boolean onItemSelected(MenuItem item) {
        ...
        switch (action) {
            ...
            case R.id.action_trim: {
                if (!MediaSetUtils.isVideoTypeSupported(mimeType)) {
                    mActivity.showToast(R.string.VideoView_error_text_type_not_supported);
                    return true;
                }
    ...
    public void onSingleTapUp(int x, int y) {
        ...
        try {
            boolean supportedGif = MediaItem.MIME_TYPE_GIF.equalsIgnoreCase(item.getMimeType());
            //Logger.d(TAG, "onSingleTapUp -  supportedPlayGif 1 : " + supportedGif);
            if (supportedGif) {
                int w = mPhotoView.getWidth();
                int h = mPhotoView.getHeight();
                supportedGif = (Math.abs(x - w / 2) * 12 <= w) && (Math.abs(y - h / 2) * 12 <= h);
                Logger.d(TAG, "onSingleTapUp -  supportedPlayGif 2 : " + supportedGif);
                if (supportedGif) {
                    playGif(item.getContentUri());
                    return;
                }
            }
        } catch (Exception e) {
            Logger.e(TAG, "onSingleTapUp -  openGif err: " + e.getMessage());
            return;
        }
    ...
    private void playGif(Uri uri) {
        Intent intent = new Intent(GalleryUtils.ACTION_VIEW_GIF);
