...
public class AlbumPage extends ActivityState implements SelectionManager.SelectionListener,
    ...
    public static final String KEY_MEDIA_PATH = "album-media-path";
    ...
    private static final int MSG_ACTION_BAR_HIDE = 10;
    private static final int MSG_ACTION_BAR_SHOW = 100;
    ...
    private void onScrollPositionChanged(int position, int total) {
        //Logger.d(TAG, "onScrollPositionChanged - mScrollPosition: " + mScrollPosition + " position: " + position);
        if (position - mScrollPosition > 0) {
            if (mHandler.hasMessages(MSG_ACTION_BAR_HIDE)) mHandler.removeMessages(MSG_ACTION_BAR_HIDE);
            mHandler.sendEmptyMessage(MSG_ACTION_BAR_HIDE);
        } else {
            if (mHandler.hasMessages(MSG_ACTION_BAR_SHOW)) mHandler.removeMessages(MSG_ACTION_BAR_SHOW);
            mHandler.sendEmptyMessage(MSG_ACTION_BAR_SHOW);
        }
...
//        }
    ...
    private void initializeViews() {
        ...
        mAlbumView = new AlbumSlotRenderer(mActivity, mSlotView, mConfig.labelSpec, mSelectionManager, mConfig.placeholderColor);
        ...
        mSlotView.setListener(new SlotView.SimpleListener() {
            ...
            public void onScrollPositionChanged(int position, int total) {
                AlbumPage.this.onScrollPositionChanged(position, total);
            }
        });
    ...
    protected boolean onCreateActionBar(Menu menu) {
        ...
        if (mGetContent) {
            ...
            //menu.findItem(R.id.action_camera).setVisible(MediaSetUtils.isCameraSource(mMediaSetPath) && GalleryUtils.isCameraAvailable
            // (mActivity));
    ...
    private class DeleteEmptyAlbum implements ThreadPool.Job<Void> {
        ...
        public Void run(ThreadPool.JobContext jc) {
            ...
            Logger.d(TAG, "delete album form database - name: " + fileName + " isSucc: " + isSucc);
            return null;
        }
    }
    ...
    private class MoveFileTask extends AsyncTask<String, Integer, Integer> {
        ...
        public MoveFileTask(List<Path> fromPath, String toFilePath) {
